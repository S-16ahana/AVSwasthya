...
const BookApp = () => {
  ...
  const handleProceed = async () => {
    ...
    if (success) navigate("/dashboard/payment1", {
      state: { name: form.fullName, email: form.email, date: form.date, time: form.time,
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        <div className="flex-1">
          ...
          <div className="flex gap-4 mb-6"> <button
              ...
              className={` view-btn ${
                form.location === "Home Collection" ? "" : "bg-gray-200"}`}onClick={() =>
                test.type !== "Scan" && setForm({ ...form, location: "Home Collection" }) } >
              Home Collection </button>
            <button className={`btn btn-primary ${  form.location === "Visit Lab" ? "" : ""}`}
              onClick={() => setForm({ ...form, location: "Visit Lab" })} >   Visit Lab</button></div>
          {form.location === "Home Collection" && test.type !== "Scan" && (
            <div className="mb-6">
              <textarea name="address" placeholder="Enter your address"value={form.address}onChange={handleChange} className="input-field"/>
              {errors.address && (<p className="text-red-500 text-sm mt-1">{errors.address}</p>)} </div>)}
          <div className="flex gap-4 mb-6">
            <div className="flex-1">
              <input type="date"    name="date"  value={form.date}    onChange={handleChange}className="input-field"/>
                {errors.date && ( <p className="text-red-500 text-sm mt-1">{errors.date}</p> )}</div>
            <div className="flex-1">
              <input type="time" name="time"value={form.time}onChange={handleChange}className="input-field"/>
                {errors.time && (<p className="text-red-500 text-sm mt-1">{errors.time}</p>)} </div></div>
          <div className="flex gap-4 mb-6">
            <div className="flex-1">
              <input  name="fullName"placeholder="Full Name"  maxLength={20} value={form.fullName}   onChange={handleChange}  className="input-field"  />
              {errors.fullName && (<p className="text-red-500 text-sm mt-1">{errors.fullName}</p>)} </div>
            <div className="flex-1">
              <input   name="phone"placeholder="Phone Number"maxLength={10} value={form.phone} onChange={handleChange}
                className="input-field"/>{errors.phone && (
                <p className="text-red-500 text-sm mt-1">{errors.phone}</p>)} </div></div>
          <button onClick={handleProceed}disabled={loading}className="btn btn-primary w-full">
            {loading ? "Booking..." : "Proceed to Payment"}  </button> </div>
        <div className="lg:w-1/3"><div className="card-stat">
            <h3 className="h4-heading mb-4">Appointment Summary</h3>
            <div className="paragraph">
              <p><strong>Test:</strong> {test.title}</p>
              <p><strong>Lab:</strong> {lab.name}</p><p>
                <strong>Location:</strong>{" "}
                {form.location === "Home Collection" && test.type !== "Scan"
                  ? form.address || "Home address not entered"
                  : lab.location}</p>
              <hr className="my-2" />  <p><strong>Test Price:</strong> <span className="paragraph">₹{lab.price}</span></p>  <p>
                <strong>Home Collection Fee:</strong>{" "}
                <span className="paragraph">₹{form.location === "Home Collection" && test.type !== "Scan" ? "0" : "0"}</span> </p>
              <p className="h4-heading mt-4">
                Total:{" "}<span className="h4-heading">₹{lab.price}</span></p>
            </div><div className="text-sm text-gray-500 mt-4">
